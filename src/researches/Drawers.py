...
from src.researches.Cells import CircleCells
    ...
    def draw_cells_volumes(self):
        ...
        for i in range(1, self._cells.rows + 1):
            for j in range(1, self._cells.columns + 1):
                ...
                point_ids = []
                for point in points:
                    point_ids.append(self.point_service.add(point["x"], point["y"], self._body.get_zend_cells()))
...
class CircleDrawer(AbstractNAngleDrawers):
    ...
    def set_cells(self, cells: CircleCells):
        self._cells = cells
